...
//▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰//
// [DIMODIFIKASI] Perintah Owner: Backup Database JSON (4 files)
//▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰//
...
bot.command("backupdb", async (ctx) => {
    ...
    // [DIUBAH] Sebutkan semua file
    await ctx.reply("⏳ Membuat backup database `users.json`, `premusers.json`, `resellers.json`, dan `claimed_users.json`...", { parse_mode: 'Markdown' });

    try {
        // 2. Buat Timestamp (tidak berubah)
        const now = new Date();
        ...
        // [DIUBAH] Tambahkan nama file users
        const usersBackupFilename = `users_backup_${timestamp}.json`; // [BARU]
        ...
        // [BARU] Backup users.json
        try {
            ensureUserFile(); // Gunakan fungsi ensure yang sudah ada
            const userContent = fs.readFileSync(userFile); // Baca file users
            await ctx.replyWithDocument({ source: userContent, filename: usersBackupFilename }); // Kirim
            successFiles.push(usersBackupFilename);
        } catch (fileError) {
            console.error("Gagal backup users.json:", fileError);
            failedFiles.push("users.json");
            await ctx.reply(`❌ Gagal backup \`users.json\`. Error: ${fileError.message}`, { parse_mode: 'Markdown' });
        }

        // 3. Backup premusers.json (tidak berubah)
        try {
            ensurePremFile();
            ...
            await ctx.reply(`❌ Gagal backup \`premusers.json\`. Error: ${fileError.message}`, { parse_mode: 'Markdown' });
        }

        // 4. Backup resellers.json (tidak berubah)
        try {
            ensureResellerFile();
            ...
            await ctx.reply(`❌ Gagal backup \`resellers.json\`. Error: ${fileError.message}`, { parse_mode: 'Markdown' });
        }

        // 5. Backup claimed_users.json (tidak berubah)
        try {
            ensureClaimedFile();
            ...
            await ctx.reply(`❌ Gagal backup \`claimed_users.json\`. Error: ${fileError.message}`, { parse_mode: 'Markdown' });
        }

        // 6. Kirim ringkasan (tidak berubah)
        let summary = `✅ Proses Backup Selesai.\n\n`;
        if (successFiles.length > 0) {
            summary += `File berhasil dikirim:\n- ${successFiles.join('\n- ')}\n\n`;
        }
        if (failedFiles.length > 0) {
            summary += `File GAGAL dibackup:\n- ${failedFiles.join('\n- ')}`;
        }
        if (successFiles.length > 0 || failedFiles.length > 0) {
           await ctx.reply(summary);
        } else {
           await ctx.reply("Tidak ada file database yang bisa dibackup saat ini.");
        }
